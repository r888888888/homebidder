...
# ---------------------------------------------------------------------------
# AVM blend — 15% RentCast AVM weight when median_comp_anchor
# ---------------------------------------------------------------------------

class TestAvmBlend:
    def test_avm_blend_applied_when_median_comp_anchor(self):
        """fair_value = round(0.85 * comp + 0.15 * avm) before lot/sqft adjustments."""
        listing = {**BASE_LISTING, "avm_estimate": 1_400_000}
        # No lot_size/sqft in listing or stats to avoid adjustment noise
        stats = {"comp_count": 5, "median_sale_price": 1_100_000, "median_pct_over_asking": 0.0}
        result = recommend_offer(listing, stats)
        expected = round(0.85 * 1_100_000 + 0.15 * 1_400_000)  # 1_145_000
        assert result["fair_value_estimate"] == expected

    def test_avm_blend_not_applied_on_ppsf_fallback(self):
        """When method is ppsf_fallback, the AVM is ignored."""
        # No median_comp — forces ppsf_fallback
        listing_with_avm = {**BASE_LISTING, "avm_estimate": 1_400_000}
        listing_without_avm = {**BASE_LISTING}
        stats = {"median_price_per_sqft": 700.0, "median_pct_over_asking": 0.0}

        result_with = recommend_offer(listing_with_avm, stats)
        result_without = recommend_offer(listing_without_avm, stats)

        assert result_with["fair_value_estimate"] == result_without["fair_value_estimate"]
        assert result_with["fair_value_breakdown"]["method"] == "ppsf_fallback"

    def test_avm_blend_not_applied_when_avm_estimate_is_none(self):
        """When avm_estimate is None, fair_value equals the comp median (no blend)."""
        listing_no_avm = {**BASE_LISTING, "avm_estimate": None}
        stats = {"comp_count": 5, "median_sale_price": 1_100_000, "median_pct_over_asking": 0.0}
        result = recommend_offer(listing_no_avm, stats)
        assert result["fair_value_estimate"] == 1_100_000

    def test_avm_blend_not_applied_when_avm_absent_from_listing(self):
        """When avm_estimate key is missing entirely, fair_value equals comp median."""
        listing_no_key = {k: v for k, v in BASE_LISTING.items() if k != "avm_estimate"}
        stats = {"comp_count": 5, "median_sale_price": 1_100_000, "median_pct_over_asking": 0.0}
        result = recommend_offer(listing_no_key, stats)
        assert result["fair_value_estimate"] == 1_100_000

    def test_fair_value_breakdown_records_avm_blend_when_applied(self):
        """fair_value_breakdown includes avm_estimate and avm_blend_pct=15 when blend active."""
        listing = {**BASE_LISTING, "avm_estimate": 1_400_000}
        stats = {"comp_count": 5, "median_sale_price": 1_100_000, "median_pct_over_asking": 0.0}
        result = recommend_offer(listing, stats)
        breakdown = result["fair_value_breakdown"]
        assert breakdown["avm_estimate"] == 1_400_000
        assert breakdown["avm_blend_pct"] == 15

    def test_fair_value_breakdown_avm_fields_none_when_no_avm(self):
        """When no AVM is provided, breakdown records avm_estimate=None, avm_blend_pct=None."""
        listing = {**BASE_LISTING}
        stats = {"comp_count": 5, "median_sale_price": 1_100_000, "median_pct_over_asking": 0.0}
        result = recommend_offer(listing, stats)
        breakdown = result["fair_value_breakdown"]
        assert breakdown["avm_estimate"] is None
        assert breakdown["avm_blend_pct"] is None

    def test_offer_range_invariant_holds_with_avm_blend(self):
        """offer_low <= offer_recommended <= offer_high even with AVM blend applied."""
        listing = {**BASE_LISTING, "avm_estimate": 1_600_000}
        stats = {**BASE_STATS, "median_sale_price": 1_100_000}
        result = recommend_offer(listing, stats)
        assert result["offer_low"] <= result["offer_recommended"] <= result["offer_high"]

...
- Non-Bay-Area property support: `fetch_ba_value_drivers()` accepts an `is_bay_area: bool` flag. When `False`, transit (BART/Caltrain/MUNI) and schools lookups are skipped — returning `None`/`[]` instead of SF/Oakland data thousands of miles away. `_BAY_AREA_COUNTIES` frozenset in orchestrator derives the flag from the geocoded county. `compute_investment_metrics` and `InvestmentData` TS interface thread `is_bay_area` through to `InvestmentCard`, which shows "Transit and school data are only available for SF Bay Area properties." for out-of-region properties. CA hazard zones and CalEnviroScreen already return graceful `None`/`False` for out-of-state coordinates. 8 new backend tests, 3 new frontend tests.

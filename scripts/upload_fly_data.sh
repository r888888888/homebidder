...
flyctl machine update "$MACHINE_ID" --autostop off --app "$APP" --yes >/dev/null
trap 'echo "==> Re-enabling autostop..."; flyctl machine update "$MACHINE_ID" --autostop stop --app "$APP" --yes >/dev/null || true' EXIT
